...
def cramer(A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        x[i] = det_Ai / det_A
        steps.append(f"Determinante de A{i+1} (al sustituir la columna {i+1} por b): {det_Ai}")
...
def gauss_jordan(A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        Ab[i] = Ab[i] / Ab[i][i]
        steps.append(f"Paso {i+1}: Matriz después de hacer el pivote igual a 1:")
        steps.append(str(Ab))
        ...
        steps.append(f"Paso {i+1}: Matriz después de eliminar la variable en las otras ecuaciones:")
        steps.append(str(Ab))
...
def sustitucion(A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        steps.append(f"Paso {i+1}: Matriz después de eliminar la variable en la fila {i}:")
        steps.append(str(A))
        steps.append(f"Paso {i+1}: Vector b actualizado:")
        steps.append(str(b))
    ...
    steps.append("Resultado final después de sustitución hacia atrás:")
    steps.append(str(x))
...
if st.button("Resolver"):
    try:
        ...
        for method_name, method_func in methods.items():
            try:
                solution, steps = method_func(A.copy(), b.copy())
                st.write(f"\n{method_name}:")
