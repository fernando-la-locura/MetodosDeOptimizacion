...
def plot_matrix(matrix: np.ndarray, title: str) -> go.Figure:
    """
    Crea una visualización de la matriz usando plotly
    """
...
with col1:
    ...
    if st.button("Resolver", type="primary"):
        try:
            ...
            with col_matrix1:
                st.plotly_chart(plot_matrix(A, "Matriz A"))
            
            with col_matrix2:
                st.plotly_chart(plot_matrix(b.reshape(-1, 1), "Vector b"))
            ...
            for tab, (method_name, method_func) in zip(tabs, methods.items()):
                with tab:
                    try:
                        ...
                        with st.expander("Ver pasos"):
                            for i, (step, matrix) in enumerate(zip(steps, matrices)):
                                st.write(f"Paso {i+1}: {step}")
                                st.plotly_chart(plot_matrix(matrix, f"Matriz - Paso {i+1}"))
